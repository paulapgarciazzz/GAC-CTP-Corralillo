...
export default function ReporteAlimentacion({ seccion }) {
    const { detalle, totales_por_tipo: totalesPorTipo, total_general: totalGeneral } = seccion;

    return (
        <section className="space-y-3">
            <h3 className="text-lg font-bold text-foreground">Alimentación</h3>

            {detalle.length === 0 ? (
                <p className="text-sm text-foreground-faint italic">Sin asignaciones de alimentación en este periodo.</p>
            ) : (
                <>
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-surface text-foreground-soft">
                                <tr>
                                    ...
                                    <th className="text-left px-4 py-2 font-semibold whitespace-nowrap">Tipo de alimentación</th>
                                    <th className="text-left px-4 py-2 font-semibold whitespace-nowrap">Cantidad</th>
                            ...
                            <tbody className="divide-y divide-border">
                                {detalle.map((item, indice) => (
                                    <tr key={item.id_asignacion ?? indice}>
                                        ...
                                        <td className="px-4 py-2 text-foreground-soft whitespace-nowrap">{item.tipo_alimentacion ?? '—'}</td>
                    ...
                    <div className="bg-surface border border-border rounded-xl p-4 grid gap-1.5 sm:grid-cols-2 lg:grid-cols-3">
                        {totalesPorTipo.map((item) => (
                            <p key={item.tipo_alimentacion} className="text-sm text-foreground-soft">
                                <span className="font-semibold text-foreground">{item.tipo_alimentacion}:</span> {item.total}
                            </p>
                        ))}
                    </div>

                    <p className="text-sm font-semibold text-foreground">Total general de raciones: {totalGeneral}</p>
                </>
            )}
        </section>
    );
